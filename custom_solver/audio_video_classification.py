        ...
        for epoch in range(start_iter, self.n_epochs):  # 迭代循环
            ...
            #############################################################
                ...
                if (step % self.log_every == 0) and self.use_tensorboard:
                    ...
                    log_loss['train/train_audio_loss'] = audio_loss.item()
                    log_loss['train/train_video_loss'] = video_loss.item()
                    log_loss['train/train_fusion_loss'] = fusion_loss.item()
                    for tag, value in log_loss.items():
                        self.logger.scalar_summary(tag, value, step)
                elapsed = datetime.now() - start_time
                print(
                    f"# <train>: [Epoch {epoch}/{self.n_epochs}] [Batch {batch_i+1}/{len(self.train_loader)}] [lr: {self.optimizer.param_groups[0]['lr']}] [train_audio_loss: {audio_loss.item()}] [train_video_loss: {video_loss.item()}] [train_fusion_loss: {fusion_loss.item()}] [time: {elapsed}]")
            # 保存模型
            self.last_epochs = epoch  # 记录当前迭代次数
            if (epoch % self.model_save_every) == 0:  # 每 epoch 次保存一次模型
                self.save(save_dir=self.model_save_dir,
    ...
    def test(self):
        ...
        for data in self.test_loader:
            ...
            with torch.no_grad():
                ...
                audio_label_pre = torch.max(audio_out, dim=1)[1]  # 获取概率最大值位置
                video_label_pre = torch.max(video_out, dim=1)[1]  # 获取概率最大值位置
                fusion_label_pre = torch.max(fusion_out, dim=1)[1]
        ...
        fusion_eval_accuracy = accuracy_score(
            fusion_total_labels.cpu(), fusion_label_preds.cpu())  # 计算准确率
        fusion_eval_loss = fusion_total_loss.mean().item()
        ...
        print(
            f"# <test>: [video_eval_loss: {video_eval_loss}] [video_eval_accuracy: {video_eval_accuracy}]")
        print(
            f"# <test>: [fusion_eval_loss: {fusion_eval_loss}] [fusion_eval_accuracy: {fusion_eval_accuracy}]")
        ...
        if self.use_tensorboard:
            ...
            self.logger.scalar_summary(
                "Val/Video_Accuracy_Eval", video_eval_accuracy, self.last_epochs)
            self.logger.scalar_summary(
                "Val/Fusion_Loss_Eval", fusion_eval_loss, self.last_epochs)
            self.logger.scalar_summary(
                "Val/Fusion_Accuracy_Eval", fusion_eval_accuracy, self.last_epochs)

        self.accuracylog.append(
            [audio_eval_accuracy, video_eval_accuracy, fusion_eval_accuracy])
