...
class ReversibleMViT(nn.Module):
    ...
    def __init__(self,
        ...
        self.pre_q_fusion = pre_q_fusion
        dpr = [
        ...
        for i in range(len(head_mul)):
            head_mul_[head_mul[i][0]] = head_mul[i][
                1
            ]
        ...
        for i in range(depth):
            ...
            embed_dim = round_width(
                embed_dim, dim_mul_[i - 1] if i > 0 else 1.0, divisor=num_heads
            )
            dim_out = round_width(
                embed_dim,
                dim_mul_[i],
...
class StageTransitionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.drop_path_rate = droppath_rate
        ...
        self.cls_embed_on = has_cls_embed
