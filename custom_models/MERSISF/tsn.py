...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        ).in_features  # 记录原基本模型的最终全连接层输入参数
        if self.dropout == 0:
            setattr(
                self.base_model,
                self.base_model.last_layer_name,
                nn.Linear(feature_dim, num_class),
            )  # 修改原基本模型的最终全连接层参数(主要是修改输出参数和重新训练)
            self.new_fc = None
        else:
            setattr(
                self.base_model,
    ...
    def _construct_flow_model(self, base_model):
        ...
        new_kernel_size = (
            kernel_size[:1] + (2 * self.new_length,) + kernel_size[2:]
        ...
        new_kernels = (
            ...
            .contiguous()
        )  # 利用原卷积核 in_c 维度的平均值填充新通道 new_in_c 维度
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        if not keep_rgb:  # 仅使用 diff 模式
            new_kernel_size = (
                kernel_size[:1] + (3 * self.new_length,) + kernel_size[2:]
            ...
            new_kernels = torch.cat(
                ...
                1,
            )  # 利用原卷积核 in_c 维度的平均值填充新通道 in_c+new_in_c 维度
            new_kernel_size = (
