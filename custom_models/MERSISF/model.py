...
class BER_SISF(nn.Module):
    ...
    def forward(self, af: Tensor, vf: Tensor, af_len=None, vf_len=None) -> Tensor:
        ...
        if self.enable_classifier:
            ...
            return joint_fea, [common_feature, af_fea, vf_fea, common_p, af_p, vf_p]
...
class TransformerFusion(nn.Module):
    ...
    def forward(self, fusion_fea):
        fusion_fea = fusion_fea.permute(1, 0, 2)  # [seg_len, B, 2*F]
        transformer_out = self.transformer_encoder(fusion_fea)  # [seg_len, B, 2*F]
        return transformer_out.permute(1, 0, 2)  # [B, seg_len, 2*F]

class BER_SISF_E_L(nn.Module):
    """
    paper: MER_SISF: Multimodal interframe feature fusion network
    """
    ...
    def __init__(
        ...
        self.fusion_feature = nn.Sequential(
            TransformerFusion(embed_dim=self.af_last_hidden_dim + self.vf_last_hidden_dim, num_heads=4, num_layers=2),
            nn.Linear(in_features=self.af_last_hidden_dim + self.vf_last_hidden_dim,out_features=mid_hidden_dim,),
            nn.ReLU(),
            nn.LayerNorm(mid_hidden_dim)
        )
...
class BER_SISF_E_P(nn.Module):
    ...
    def forward(self, af: Tensor, vf: Tensor, af_len=None, vf_len=None) -> Tensor:
        ...
        # 分类
        if self.enable_classifier:
            outs = self.classifier2(joint_fea), self.classifier5(joint_fea), self.classifier6(joint_fea)
            return outs, sim_loss, torch.tensor(0.0)
        else:
            return joint_fea
