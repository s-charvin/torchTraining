...
class Conv1dSame(torch.nn.Conv1d):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if pad_sq > 0:
            x = F.pad(x, [pad_sq // 2, pad_sq - pad_sq // 2]).contiguous()
        return F.conv1d(
            x,
            self.weight,
            self.bias,
...
class TIMNet(nn.Module):
    def __init__(
        ...
        self.last_linear = nn.Linear(nb_filters, num_classes)
    ...
    def forward(self, x, x_len=None):
        ...
        for i in range(self.nb_stacks):
            for j in range(self.dilations):
                skip_out_forward = self.temporal_aware_blocks_forward[
                    i * self.dilations + j
                ](skip_out_forward)
                skip_out_backward = self.temporal_aware_blocks_backward[
                    i * self.dilations + j
                ](skip_out_backward)
