...
class EvoSABlock(nn.Module):
    ...
    def forward(self, cls_token, x):
        ...
        if self.prune_ratio == 1:
            ...
            x_sorted, global_attn, token_indices = (
                x_ga_ti[:, :, :-2],
                x_ga_ti[:, :, -2],
                x_ga_ti[:, :, -1],
            )
            ...
            x_ga_ti = torch.cat(
                (x_sorted, global_attn.unsqueeze(-1), token_indices.unsqueeze(-1)),
                dim=-1,
            )
            x_ga_ti = easy_gather(x_ga_ti, indices)
            x_patch, global_attn, token_indices = (
                x_ga_ti[:, :, :-2],
                x_ga_ti[:, :, -2],
                x_ga_ti[:, :, -1],
            )
...
class Visionformer(nn.Module):
    """Vision Transformer
    Transformers for Image Recognition`  - https://arxiv.org/abs/2010.11929
    """

    def __init__(
        self,
        depth=[2, 5, 8, 2],  # cfg.UNIFORMER.DEPTH 模型深度
        num_classes=400,  # cfg.MODEL.NUM_CLASSES 分类类别数
        in_chans=3,  # cfg.DATA.INPUT_CHANNEL_NUM[0] 输入通道数
        embed_dim=[56, 112, 224, 448],  # cfg.UNIFORMER.EMBED_DIM 嵌入维度
        head_dim=28,  # cfg.UNIFORMER.HEAD_DIM 注意力头维度
        mlp_ratio=3,  # cfg.UNIFORMER.MLP_RATIO MLP扩展比例
        qkv_bias=True,  # cfg.UNIFORMER.QKV_BIAS QKV是否使用偏置
        qk_scale=None,  # cfg.UNIFORMER.QKV_SCALE QKV缩放因子
        representation_size=None,  # cfg.UNIFORMER.REPRESENTATION_SIZE 表示层维度
        drop_rate=0,  # cfg.UNIFORMER.DROPOUT_RATE Dropout率
        attn_drop_rate=0,  # cfg.UNIFORMER.ATTENTION_DROPOUT_RATE 注意力Dropout率
        drop_path_rate=0.0,  # cfg.UNIFORMER.DROP_DEPTH_RATE DropPath率
        prune_ratio=[
            [],
            [],
            [1, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
            [0.5, 0.5, 0.5],
        ],  # cfg.UNIFORMER.PRUNE_RATIO 剪枝比例
        trade_off=[
            [],
            [],
            [1, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
            [0.5, 0.5, 0.5],
        ],  # cfg.UNIFORMER.TRADE_OFF 交换比例
    ):
        super().__init__()
        ...
        for name, p in self.named_parameters():
            # 在这里将proj权重填充为1，以改善训练动态。否则，时间注意力的输入将乘以0*0，模型很难移出这种状态。
    ...
    def get_pretrained_model(self, name="uniformer_xxs_160_in1k"):
    ...
    def forward_features(self, x):
        ...
        token_indices = torch.arange(
            x.shape[2] * x.shape[3] * x.shape[4], dtype=torch.long, device=x.device
        ).unsqueeze(0)
...
if __name__ == "__main__":
    # 创建模型实例
    model = Visionformer()
    input = torch.randn(1, 3, 10, 250, 250)
    output = model(input)
