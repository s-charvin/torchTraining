...
class GaussianLowPass(nn.Module):
    ...
    def forward(self, x):
        kernel = impulse_responses.gaussian_lowpass(
            self.weights, self.kernel_size)
        ...
        outputs = F.conv1d(x, kernel, bias=self._bias, stride=self.strides,
                           padding=pad_val, groups=self.in_channels)
        return outputs
