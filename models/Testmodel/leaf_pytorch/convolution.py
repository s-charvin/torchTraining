...
from .initializers import GaborInit
...
class GaborConstraint(nn.Module):
    ...
    def forward(self, kernel_data):
        ...
        sigma_upper = self._kernel_size * \
            torch.sqrt(
                2. * torch.log(torch.tensor(2., device=kernel_data.device))) / math.pi
        clipped_mu = torch.clamp(
            kernel_data[:, 0], mu_lower, mu_upper).unsqueeze(1)
        clipped_sigma = torch.clamp(
            kernel_data[:, 1], sigma_lower, sigma_upper).unsqueeze(1)
    ...
    def forward(self, x):
        ...
        if self._sort_filters:
            raise NotImplementedError(
                "sort filter functionality not yet implemented")
        filters = gabor_filters(kernel, self._kernel_size,
                                legacy_complex=self.use_legacy_complex)
