...
_POOL1 = {"slow": [[1, 1]], "fast": [
    [1, 1]], "slowfast": [[1, 1], [1, 1]], }
...
class SlowFast(nn.Module):
    ...
    def _construct_network(self, modelarch, stagelayers):
        ...
        out_dim_ratio = (  # 计算得到输出特征图维度比
            BETA_INV // FUSION_CONV_CHANNEL_RATIO  # = 4
        )
        ...
        self.head = components.ResNetBasicHead(
            ...
            num_classes=NUM_CLASSES if len(
                NUM_CLASSES) > 1 else NUM_CLASSES[0],
            pool_size=[
                [
                    NUM_FRAMES
                    // ALPHA // 4
...
def slowfast50() -> SlowFast:
    return SlowFast("slowfast",  [3, 4, 6, 3])


def slowfast101() -> SlowFast:
    return SlowFast("slowfast", [3, 4, 23, 3],)
